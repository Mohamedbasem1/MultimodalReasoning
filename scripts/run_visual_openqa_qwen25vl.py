...
try:
    ...
    raise ImportError(
        "Qwen2_5_VLForConditionalGeneration is missing. Install the Qwen2.5-VL requirements first."
    ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Qwen2.5-VL-7B-Instruct on ImageCLEF Visual OpenQA.")
    parser.add_argument("--model", default=DEFAULT_MODEL)
    parser.add_argument("--adapter", default=None, help="Optional PEFT/LoRA adapter directory.")
    parser.add_argument("--dataset", default=DEFAULT_DATASET)
    parser.add_argument("--split", default="test")
    parser.add_argument("--output", default="outputs/visual_openqa_qwen25vl7b_instruct.json")
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        print("Warning: CUDA is not available. Qwen2.5-VL-7B inference will be very slow on CPU.")
    ...
    with raw_output_path.open("w", encoding="utf-8") as raw_file:
        for row in tqdm(dataset, desc="Qwen2.5-OpenQA"):
