...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="QLoRA fine-tune Qwen3.6-35B-A3B Unsloth on Visual MCQ.")
    parser.add_argument("--model", default=DEFAULT_MODEL)
    parser.add_argument("--init-adapter", default=None, help="Optional existing adapter to continue from.")
    parser.add_argument("--dataset", default=DEFAULT_DATASET)
    parser.add_argument("--train-split", default="train")
    parser.add_argument("--output-dir", default="outputs/qwen36-35b-a3b-mcq-unsloth-lora")
    ...
    parser.add_argument("--max-steps", type=int, default=-1, help="-1 means train for num_epochs over the dataset.")
...
def filter_dataset(dataset: Dataset, args: argparse.Namespace, answer_column: str) -> Dataset:
    ...
    dataset = dataset.filter(lambda row: normalize_answer(row.get(answer_column)) in ANSWER_KEYS)
    return dataset
...
def convert_to_conversation(row: Dict[str, Any], prompt: str, image_column: str, answer_column: str, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    answer = normalize_answer(row[answer_column])
    if answer is None:
        raise ValueError(f"Cannot convert row without a valid answer: {row.get(answer_column)!r}")
    return {
        "messages": [
            {
                "role": "user",
...
def main() -> None:
    ...
    answer_column = pick_column(dataset.column_names, args.answer_column, ["answer_key", "label", "answer"])
    print(f"ID column: {id_column}")
    print(f"Image column: {image_column}")
    print(f"Answer column: {answer_column}")

    dataset = maybe_limit(filter_dataset(dataset, args, answer_column), args.train_limit)
    ...
    model_name = args.init_adapter or args.model
